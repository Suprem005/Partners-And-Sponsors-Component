...
export default function AboutOrganizer({
  ...
  return (
    <section className="@container py-20 border-b border-gray-200 bg-[url('/images/organization/wallpaper.jpg')] bg-no-repeat bg-center bg-cover">
      {/* title and short description part  */}
      <div className={`${className} ${TextPositionClass[textPosition]}`}>
        <h1 className="text-3xl text-white @md:text-5xl font-bold mb-6 leading-tight">
          {title}
        </h1>
        <p className="text-white text-lg mb-8">{shortDescription}</p>
      </div>
      ...
      >
        ...
        <div className="w-full @md:w-1/2 flex flex-col justify-center gap-8">
          ...
          >
            <p className={`text-white  ${TextAlignmentClass[textAlignment]}`}>
              {bodyDescription}
            </p>
            <p
              className={`text-white line-clamp-5 ${TextAlignmentClass[textAlignment]}`}
            >
              {bodyDescription}
            </p>
          </div>
